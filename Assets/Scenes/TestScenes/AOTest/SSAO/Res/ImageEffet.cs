...
public class ImageEffet : MonoBehaviour
{
    ...
    public bool OnlyShowViewNormal = false;
    ...
    private Material _viewNormalMat;
    private void Awake()
    {
        ...
        var viewNormalShader = Shader.Find("Test/ViewNormalShader");
        _viewNormalMat = new Material(viewNormalShader);
    }
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if (OnlyShowViewNormal)
        {
            //ViewNormal
            // RenderTexture viewNormalRT = RenderTexture.GetTemporary(source.width, source.height, 0);
            Graphics.Blit(source, destination, _viewNormalMat);
            // RenderTexture.ReleaseTemporary(viewNormalRT);
        }
        else
        {
            //AO
